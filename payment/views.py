...
class CustomerClientTransactionListAPIView(APIView):
    permission_classes = [IsClient]  # Ensure only customer users can access
    ...
    def get(self, request):
        try:
            ...
            # Get all users under this ISP
            users_under_isp = User.objects.filter(isp=request.user.isp)
...
class ISPAdminTransactionListAPIView(APIView):
    permission_classes = [IsISP]  # Ensure only ISP users can access
    ...
    def get(self, request):
        try:
            ...
            status_filter = request.query_params.get('status')
            isp_filter = request.query_params.get('isp')
            search_term = request.query_params.get('search')  # For searching username/email
...
class CustomerAdminTransactionListAPIView(APIView):
    permission_classes = [IsClient]  # Ensure only customer users can access
    ...
    def get(self, request):
        try:
            ...
            if not request.user.isp:
                return Response({
                    "status": False,
                    "message": "Customer does not have an associated ISP"
                }, status=status.HTTP_400_BAD_REQUEST)
